# Model the Device
The first step to creating an IoT solution in a health monitoring use case is to model the device, or the car, in the IoT platform. A device template is used to represent a device of a particular kind and create multiple instances of it. A device template contains Attribute Type, Event Template and Command Template. In this case, the device template for a car will have Attribute Types and Event Templates. Since no commands are sent out to the car, Command Templates are not modeled. 
...
Bind the event field to attribute so that the attribute is updated with the latest value in the event. The bound attribute must have the same data type as the field. With the eventID and the attributeID obtained in the previous requests, create a binding between the event and the attribute using this web service.
